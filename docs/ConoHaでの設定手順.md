...
## ⚠️ 最初に：コマンドは「サーバーの中」で実行します

この手順書のコマンドは、**すべてConoHaのサーバー上で実行するもの**です。
自分のパソコンのPowerShellやターミナルにそのまま貼り付けても動きません
（`find` や `grep` はLinuxのコマンドで、Windowsには存在しないためです）。

**今どちらにいるかは、行頭の表示（プロンプト）で見分けられます。**

| 行頭の表示 | 状態 |
|---|---|
| `PS C:\Users\xxxx>` | ❌ 自分のパソコン。Linuxコマンドは使えません |
| `root@サーバー名:~#` | ✅ ConoHaサーバーの中。ここでコマンドを打ちます |

まず手順0でサーバーに接続し、**表示が `root@...` に変わってから**先に進んでください。
...
Windowsなら「PowerShell」、Macなら「ターミナル」を開いて、**まず接続します**：

```bash
ssh root@（サーバーのIPアドレス）
```

- IPアドレス: ConoHa管理画面 →「サーバー」→ サーバー名 →「ネットワーク情報」に表示
- 初回は `Are you sure you want to continue connecting?` と聞かれます → `yes` + Enter
- `password:` は**入力しても画面に表示されません**が、打ててはいます。そのままEnter

接続に成功すると、行頭の表示が `PS C:\Users\xxxx>` から
`root@サーバー名:~#` に**変わります**。これが「サーバーの中に入れた」合図です。

> ❌ よくある間違い: PowerShellを開いただけで、`ssh` を実行せずに
> 診断コマンドを貼り付けてしまう。この場合は自分のPCで実行されるため、
> `grep は認識されません` `systemctl は認識されません` といったエラーになります。
> その場合は、上の `ssh` から実行し直してください。
...
> ✅ 実行前に、行頭が `root@...` になっていることを確認してください。
> `PS C:\Users\xxxx>` のままなら、まだ自分のPCです（手順0に戻ってください）。
...
| `grep は認識されません` 等のエラー | 自分のPCで実行しています。手順0でサーバーに接続してから実行してください |
