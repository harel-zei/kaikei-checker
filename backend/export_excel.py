...
def _build_checksheet(wb, issues, period):
    ...
    ws.merge_range("A3:H3",
        "【操作方法】　G列（対応済）のセルをクリック → ☑ を選択 → 行がグレーアウトされます。解除は ☐ を選択してください。",
        fmt_instr)

    # ── Row4: 列ヘッダー ──
    headers = ["勘定科目", "補助科目", "日付", "伝票No.", "確認内容（Harel入力欄）", "ご回答", "対応済", "備考"]
    for col, h in enumerate(headers):
        ws.write(3, col, h, fmt_check_header if col == 6 else fmt_header)
    ...
    ws.conditional_format(4, 0, last_data_row, 7, {
        "type":     "formula",
        "criteria": f'=$G5="{CHECK_CHAR}"',
        "format":   done_fmt,
    })
    ...
    for i, issue in enumerate(issues):
        ...
        slip = str(issue.get("slip", "") or "")

        ws.write(row, 0, display_base, f_n)
        ws.write(row, 1, sub,          f_n)
        ws.write(row, 2, date_str,     f_c)
        ws.write(row, 3, slip,         f_c)           # 伝票No.
        ws.write(row, 4, clean_msg,    f_w)
        ws.write(row, 5, "",           f_n)           # ご回答
        ws.write(row, 6, UNCHECK_CHAR, f_c)           # 対応済（初期値 ☐）
        ws.write(row, 7, "",           f_n)           # 備考

        # ドロップダウン: ☑ か ☐ を選択
        ws.data_validation(row, 6, row, 6, {
            "validate": "list",
...
def _build_sample_sheet(wb):
    ...
    ws.set_column("C:C", 12); ws.set_column("D:D", 12)
    ws.set_column("E:E", 62); ws.set_column("F:F", 26)
    ws.set_column("G:G", 10); ws.set_column("H:H", 20)
    ws.set_row(0, 28); ws.set_row(1, 18); ws.set_row(2, 22)

    ws.merge_range("A1:H1", "会計データ確認シート", fmt_t)
    ws.merge_range("A2:H2", "対象月：　　　　年　　　月分　　　　　　　　　　　　　　　担当：　　　　　　　",
        wb.add_format({"font_name": "メイリオ", "font_size": 11}))
    for col, h in enumerate(["勘定科目","補助科目","日付","伝票No.","確認内容（Harel入力欄）","ご回答","対応済","備考"]):
        ws.write(2, col, h, fmt_h)

    # 条件付き書式
    done_fmt = wb.add_format({"font_color": C_DONE_FG, "bg_color": C_DONE_BG, "font_name": "メイリオ"})
    ws.conditional_format(3, 0, 15, 7, {
        "type": "formula", "criteria": f'=$G4="{CHECK_CHAR}"', "format": done_fmt,
    })
    ...
    for i, (acc, sub, date, slip, content, chk) in enumerate(samples):
        row = 3 + i
        is_alt = i % 2 == 1
        f_n = fa if is_alt else fw
        f_w = fa_w if is_alt else fw_w
        ...
        ws.write(row, 3, slip,    f_c)
        ws.write(row, 4, content, f_w)
        ws.write(row, 5, "",      f_n)
        ws.write(row, 6, chk,     f_c)
        ws.write(row, 7, "",      f_n)
        ws.data_validation(row, 6, row, 6, {
    ...
    ws.merge_range(3+len(samples)+1, 0, 3+len(samples)+1, 7,
        "【操作方法】　G列（対応済）のセルをクリック → ☑ を選択 → 行がグレーアウト。解除は ☐ を選択。",
